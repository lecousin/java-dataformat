...
public class DIBReader {
	...
	public static AsyncSupplier<Integer, Exception> readHeader(IO.Readable input, DIBHeader header, long pixelsDataOffsetFromDIBHeader) {
		AsyncSupplier<Integer, Exception> result = new AsyncSupplier<>();
		byte[] b = new byte[4];
		AsyncSupplier<Integer, IOException> readHeaderSize = input.readFullyAsync(ByteBuffer.wrap(b));
		readHeaderSize.onDone(new Runnable() {
			@Override
			public void run() {
				...
				int size = (int)DataUtil.Read32U.LE.read(b, 0);
				...
				AsyncSupplier<Integer, IOException> readHeader = input.readFullyAsync(ByteBuffer.wrap(bh));
				readHeader.thenStart("Read DIB header from " + input.getSourceDescription(), input.getPriority(), (Task<Void, NoException> t) -> {
					int off;
					if (size == 12) {
						header.version = DIBHeader.Version.OS2_1;
						header.width = DataUtil.Read16.LE.read(bh, 0);
						header.height = DataUtil.Read16.LE.read(bh, 2);
						off = 4;
					} else {
						header.width = DataUtil.Read32.LE.read(bh, 0);
						header.height = DataUtil.Read32.LE.read(bh, 4);
						off = 8;
					}
					header.planes = DataUtil.Read16U.LE.read(bh, off);
					off += 2;
					// planes is supposed to be 1, but do not throw an error if not...
					header.bitsPerPixel = DataUtil.Read16U.LE.read(bh, off);
					off += 2;
					...
					if (off < size - 4) {
						header.compression = DataUtil.Read32U.LE.read(bh, off);
						off += 4;
					}
					if (off < size - 4) {
						header.bitmapSize = DataUtil.Read32U.LE.read(bh, off);
						off += 4;
					}
					if (off < size - 4) {
						header.horizRes = DataUtil.Read32.LE.read(bh, off);
						off += 4;
					}
					if (off < size - 4) {
						header.vertRes = DataUtil.Read32.LE.read(bh, off);
						off += 4;
					}
					if (off < size - 4) {
						header.colorsUsed = DataUtil.Read32U.LE.read(bh, off);
						off += 4;
					}
					if (off < size - 4) {
						header.importantColorsUsed = DataUtil.Read32U.LE.read(bh, off);
						off += 4;
					}
					...
					if (size > 40 && size <= 64 && size != 52 && size !=56) {
						...
						if (off < size - 4) {
							header.resUnit = DataUtil.Read16U.LE.read(bh, off);
							off += 2;
						}
						if (off < size - 4)
							off += 2; // 2 bytes reserved
						if (off < size - 4) {
							header.recording = DataUtil.Read16U.LE.read(bh, off);
							off += 2;
						}
						if (off < size - 4) {
							header.rendering = DataUtil.Read16U.LE.read(bh, off);
							off += 2;
						}
						if (off < size - 4) {
							header.renderingSize1 = DataUtil.Read32U.LE.read(bh, off);
							off += 4;
						}
						if (off < size - 4) {
							header.renderingSize2 = DataUtil.Read32U.LE.read(bh, off);
							off += 4;
						}
						if (off < size - 4) {
							header.colorEncoding = DataUtil.Read32U.LE.read(bh, off);
							off += 4;
						}
						if (off < size - 4) {
							header.appIdentifier = DataUtil.Read32U.LE.read(bh, off);
							off += 4;
						}
						...
						if (size < 52 && (header.compression == 3 || header.compression == 6)) {
							...
							header.redMask = DataUtil.Read32U.LE.read(bh, 0);
							header.greenMask = DataUtil.Read32U.LE.read(bh, 4);
							header.blueMask = DataUtil.Read32U.LE.read(bh, 8);
							off += 12;
							if (header.compression == 6) {
								header.alphaMask = DataUtil.Read32U.LE.read(bh, 12);
								off += 4;
							}
						} else if (size >= 52) {
							header.redMask = DataUtil.Read32U.LE.read(bh, off);
							off += 4;
							header.greenMask = DataUtil.Read32U.LE.read(bh, off);
							off += 4;
							header.blueMask = DataUtil.Read32U.LE.read(bh, off);
							off += 4;
						}
						if (size >= 56) {
							header.alphaMask = DataUtil.Read32U.LE.read(bh, off);
							off += 4;
						}
						if (size >= 60) {
					...
					if (size == 12) {
						if (header.bitsPerPixel <= 8) {
							...
							AsyncSupplier<Integer, IOException> readPalette = input.readFullyAsync(ByteBuffer.wrap(data));
							int nb = nbEntries;
							int offset = off + data.length;
							Task<IndexColorModel, Exception> createCM = Task.cpu("Create IndexColorModel", input.getPriority(), taskContext -> {
								byte[] r = new byte[nb];
								byte[] g = new byte[nb];
								byte[] b = new byte[nb];
								for (int i = 0; i < nb; ++i) {
								...
								int sz = nb > (1 << header.bitsPerPixel) ? 1 << header.bitsPerPixel : nb;
								return new IndexColorModel(header.bitsPerPixel, sz, r, g, b);
							});
							header.palette = createCM.getOutput();
							readPalette.onDone(new Runnable() {
								@Override
								public void run() {
	...
	public static AsyncSupplier<BufferedImage,Exception> readBitmap(DIBHeader header, IO.Readable input) {
		...
		try {
			if (header.compression == 0) {
				...
				// RLE8
				if (header.bitsPerPixel != 8)
					return new AsyncSupplier<>(null, new Exception("Invalid DIB: RLE8 compression specified, but " + header.bitsPerPixel + " bits per pixel specified while only 8 bits is allowed"));
				reader = new RLE8Reader(header);
			} else if (header.compression == 2) {
				// RLE4
				if (header.bitsPerPixel != 4)
					return new AsyncSupplier<>(null, new Exception("Invalid DIB: RLE4 compression specified, but " + header.bitsPerPixel + " bits per pixel specified while only 4 bits is allowed"));
				...
				if (!DIBHeader.Version.OS2_2.equals(header.version) || header.bitsPerPixel != 24) {
					...
					Task<BufferedImage, Exception> task = Task.cpu("Read JPEG inside BMP", input.getPriority(), t -> {
				...
				if (header.bitmapSize < 16 * 1024 * 1024) {
					...
					AsyncSupplier<Integer, IOException> read = input.readFullyAsync(ByteBuffer.wrap(data));
					AsyncSupplier<BufferedImage, Exception> result = new AsyncSupplier<>();
					read.thenStart("Read PNG inside BMP", input.getPriority(), (Task<Void, NoException> t) -> {
						...
						PNGReader.readFromBuffered(io).forward(result);
						return null;
					}, true);
					return result;
				}
		...
		if (reader == null)
			return new AsyncSupplier<>(null, new Exception("Unknown DIB fomat: " + header.version + " compression " + header.compression + " " + header.bitsPerPixel + " bits per pixel"));
