...
public abstract class ICOCURFormat extends ImageDataFormat.Multiple {
	...
	public static ImageDirectory readImageDirectory(IO.Readable.Buffered io, ICOCURFormat format) throws IOException {
		if (format == null) {
			...
			io.skip(4);
		int nbImages = DataUtil.Read16U.LE.read(io);
		...
		for (int i = 0; i < nbImages; ++i) {
			...
			if (format == ICODataFormat.instance) {
				((Icon)img).colorPlanes = DataUtil.Read16U.LE.read(io);
				if (((Icon)img).colorPlanes > 1) throw new EOFException("Invalid number of color planes");
				((Icon)img).bitsPerPixel = DataUtil.Read16U.LE.read(io);
			} else {
				((Cursor)img).hotspotX = DataUtil.Read16U.LE.read(io);
				if (((Cursor)img).hotspotX > img.width) throw new EOFException("Invalid cursor hotspot X position");
				((Cursor)img).hotspotY = DataUtil.Read16U.LE.read(io);
				if (((Cursor)img).hotspotY > img.height) throw new EOFException("Invalid cursor hotspot Y position");
			}
			img.dataSize = DataUtil.Read32U.LE.read(io);
			img.dataOffset = DataUtil.Read32U.LE.read(io);
			dir.images.add(img);
		}
		return dir;
	}
	...
	public static OpenedDataCache<ImageDirectory> cache = new OpenedDataCache<ImageDirectory>(ImageDirectory.class, 5*60*1000) {
		...
		@Override
		protected AsyncSupplier<ImageDirectory,Exception> open(Data data, IO.Readable io, WorkProgress progress, long work) {
			Task<ImageDirectory, Exception> task = Task.cpu("Read icon directory", Task.Priority.NORMAL, t -> {
	...
	public static class ICOImageProvider implements DIBImageProvider {
		...
		public <T extends IO.Readable.Buffered & IO.Readable.Seekable> AsyncSupplier<BufferedImage, Exception> provide(T io) {
			...
			readHeader.thenStart("Read ICO", io.getPriority(), () -> {
	...
	public WorkProgress listenSubData(Data container, CollectionListener<Data> listener) {
		...
		AsyncSupplier<CachedObject<ImageDirectory>,Exception> get = cache.open(container, this, Task.Priority.IMPORTANT, progress, 800);
		Task<Void,NoException> task = Task.cpu("Create sub-data from ico file", Task.Priority.IMPORTANT, t -> {
			if (get.hasError()) {
	...
	public static <T extends IO.Readable.Buffered & IO.Readable.Seekable> byte[] createBMPHeader(T dib, long dibSize, Image i) throws IOException {
		long dibHeaderSize = DataUtil.Read32U.LE.read(dib);
		byte[] header = new byte[14 + (int)dibHeaderSize];
		header[0] = 'B';
		header[1] = 'M';
		DataUtil.Write32U.LE.write(header, 2, dibSize + 14);
		DataUtil.Write32U.LE.write(header, 14, dibHeaderSize);
		dib.read(header, 14 + 4, (int)dibHeaderSize - 4);
		long offset = 14;
		offset += dibHeaderSize;
		if (dibHeaderSize == 12) {
			DataUtil.Write16U.LE.write(header, 14 + 4 + 2, i.height);
			int bitsPerPixel = DataUtil.Read16U.LE.read(header, 14 + 4 + 2 + 2 + 2);
			if (bitsPerPixel < 24)
				offset += (1 << bitsPerPixel) * 3; // palette data size
		} else {
			DataUtil.Write32U.LE.write(header, 14 + 4 + 4, i.height);
			dib.skip(10);
			int bitsPerPixel = DataUtil.Read16U.LE.read(header, 28);
			long compression = DataUtil.Read32U.LE.read(header, 30);
			if (compression == 3)
				offset += 12; // bit mask
			long colorsUsed = DataUtil.Read32U.LE.read(header, 46);
			if (colorsUsed == 0 && bitsPerPixel < 16)
				colorsUsed = (1 << bitsPerPixel);
			offset += colorsUsed * 4; // palette data
		}
		DataUtil.Write32U.LE.write(header, 10, offset);
		return header;
	}
