...
public abstract class ScanLineReader extends ImageReader {
	...
	public static ScanLineReader createIndexed(int width, int height, int bits, int bytesPerLine, boolean bottomUp, AsyncWork<IndexColorModel, Exception> colorModel) throws InvalidImage {
		if (bits < 1 || bits > 8)
	...
	public static ScanLineReader createIndexed(int width, int height, int bits, AsyncWork<IndexColorModel, Exception> colorModel, ScanLineHandler scanLineHandler) throws InvalidImage {
		if (bits < 1 || bits > 8)
			throw new InvalidImage("Invalid bit depth for indexed color image: "+bits);
	...
	public static ScanLineReader createGreyscale(int width, int height, int bits, PixelSampleModifier sampleModifier, ScanLineHandler scanLineHandler, boolean littleEndian, byte priority) throws InvalidImage {
		return createGreyscale(width, height, bits, -1, sampleModifier, scanLineHandler, littleEndian, priority);
	}
	public static ScanLineReader createGreyscale(int width, int height, int bits, int transparentPixel, PixelSampleModifier sampleModifier, ScanLineHandler scanLineHandler, boolean littleEndian, byte priority) throws InvalidImage {
	...
	public static ScanLineReader createGreyscaleWithAlpha(int width, int height, int bits, PixelSampleModifier sampleModifier, ScanLineHandler scanLineHandler, boolean littleEndian, byte priority) throws InvalidImage {
		if (bits != 8 && bits != 16)
		...
		Task<ComponentColorModel, Exception> createCM = new Task.Cpu<ComponentColorModel, Exception>("Create color model", priority) {
			@Override
			public ComponentColorModel run() {
				return new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_GRAY), new int[] {16,16}, true, false, Transparency.TRANSLUCENT, DataBuffer.TYPE_USHORT);
			}
		};
		createCM.start();
	...
	public static ScanLineReader createTrueColor16Bits(int width, int height, int bytesPerLine, int redMask, int greenMask, int blueMask, int alphaMask, boolean littleEndian, boolean bottomUp, byte priority) {
		Task<DirectColorModel, Exception> createCM = new Task.Cpu<DirectColorModel, Exception>("Create color model", priority) {
			...
			public DirectColorModel run() {
				return new DirectColorModel(16, redMask, greenMask, blueMask, alphaMask);
			}
		};
		createCM.start();
	...
	public static ScanLineReader createTrueColor32Bits(int width, int height, int bytesPerLine, int redMask, int greenMask, int blueMask, int alphaMask, boolean littleEndian, boolean bottomUp, byte priority) {
		Task<DirectColorModel, Exception> createCM = new Task.Cpu<DirectColorModel, Exception>("Create color model", priority) {
			@Override
			public DirectColorModel run() {
				return new DirectColorModel(32, redMask, greenMask, blueMask, alphaMask);
			}
		};
		createCM.start();
	...
	public static ScanLineReader createTrueColor24Bits(int width, int height, int bytesPerLine, boolean bottomUp, int[] rgbOrder, byte priority) {
		Task<ComponentColorModel, Exception> createCM = new Task.Cpu<ComponentColorModel, Exception>("Create color model", priority) {
			@Override
			public ComponentColorModel run() {
	...
	public static ScanLineReader createTrueColor(int width, int height, int bitsPerComponent, PixelSampleModifier sampleModifier, ScanLineHandler scanLineHandler, boolean littleEndian, byte priority) throws InvalidImage {
		return createTrueColor(width, height, bitsPerComponent, -1, -1, -1, sampleModifier, scanLineHandler, littleEndian, priority);
	}
	public static ScanLineReader createTrueColor(int width, int height, int bitsPerComponent, int transparentR, int transparentG, int transparentB, PixelSampleModifier sampleModifier, ScanLineHandler scanLineHandler, boolean littleEndian, byte priority) throws InvalidImage {
		if (bitsPerComponent == 8) {
			...
			Task<DirectColorModel, Exception> createCM = new Task.Cpu<DirectColorModel, Exception>("Create color model", priority) {
				@Override
				public DirectColorModel run() {
					return new DirectColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), 25, 0xff0000, 0x00ff00, 0x0000ff, 0x1000000, false, DataBuffer.TYPE_INT);
				}
			};
			createCM.start();
	...
	public static ScanLineReader createRGBA(int width, int height, int bitsPerComponent, PixelSampleModifier sampleModifier, ScanLineHandler scanLineHandler, boolean littleEndian, byte priority) throws InvalidImage {
		if (bitsPerComponent == 8) {
			Task<ComponentColorModel, Exception> createCM = new Task.Cpu<ComponentColorModel, Exception>("Create color model", priority) {
				@Override
				public ComponentColorModel run() {
	...
	protected ScanLineReader(AsyncWork<? extends ColorModel, Exception> colorModel) {
		super(colorModel);
	}
	
	private static class FullReader extends ScanLineReader {
		public FullReader(AsyncWork<? extends ColorModel, Exception> colorModel, int dataSize) {
			super(colorModel);
		...
		protected ISynchronizationPoint<Exception> readImageData(IO.ReadableByteStream io) {
			Task.Cpu<Void, Exception> task = new Task.Cpu<Void, Exception>("Full image reader", io.getPriority()) {
				@Override
	...
	private static class ByteReader extends ScanLineReader {
		public ByteReader(AsyncWork<? extends ColorModel, Exception> colorModel, int width, int height, int bitsPerPixel, int alphaBitsPerPixel, PixelSampleModifier sampleModifier, ScanLineHandler scanner) {
			super(colorModel);
			this.width = width;
			this.height = height;
		...
		protected ISynchronizationPoint<Exception> readImageData(IO.ReadableByteStream io) {
			...
			new Task.Cpu<Void, NoException>("Scan Line Reader", io.getPriority()) {
				@Override
				public Void run() {
					int bits = width*(bitsPerPixel + alphaBitsPerPixel);
					int lineBytes = bits/8 + ((bits%8) == 0 ? 0 : 1);
	...
	private static class DirectByteReader extends ScanLineReader {
		public DirectByteReader(AsyncWork<? extends ColorModel, Exception> colorModel, int width, int height, int bytesPerLine, int bitsPerPixel, boolean bottomUp) {
			super(colorModel);
			this.height = height;
	...
	private static class UShortReader extends ScanLineReader {
		public UShortReader(AsyncWork<? extends ColorModel, Exception> colorModel, int width, int height, boolean littleEndian, int components, boolean hasAlpha, PixelSampleModifier sampleModifier, ScanLineHandler scanner) {
			super(colorModel);
			this.width = width;
			this.height = height;
				...
				public Void run() {
					for (int line = 0; line < height; ++line) {
						...
						for (int i = 0; i < width*components; ++i) {
							int sample = littleEndian ? DataUtil.readUnsignedShortLittleEndian(lineOutput, i*2) : DataUtil.readUnsignedShortBigEndian(lineOutput, i*2);
	...
	private static class DirectUShortReader extends ScanLineReader {
		public DirectUShortReader(AsyncWork<? extends ColorModel, Exception> colorModel, int width, int height, int bytesPerLine, boolean littleEndian, boolean bottomUp) {
			super(colorModel);
			this.width = width;
			this.height = height;
			this.bottomUp = bottomUp;
					...
					for (int line = 0; line < height; line++) {
						...
						if (littleEndian)
							for (int i = 0; i < width; ++i)
								buffer[off+i] = DataUtil.readShortLittleEndian(lineBuffer, i*2);
						else
							for (int i = 0; i < width; ++i)
								buffer[off+i] = DataUtil.readShortBigEndian(lineBuffer, i*2);
					}
					done.unblock();
					return null;
				}
	...
	private static class UShortRGBWithTransparentPixelReader extends ScanLineReader {
		public UShortRGBWithTransparentPixelReader(AsyncWork<? extends ColorModel, Exception> colorModel, int width, int height, int transR, int transG, int transB, boolean littleEndian, PixelSampleModifier sampleModifier, ScanLineHandler scanner) {
			super(colorModel);
			this.width = width;
			this.height = height;
			buffer = new int[width*height];
			...
			new Task.Cpu<Void, NoException>("Scan Line Reader", io.getPriority()) {
				...
				public Void run() {
					...
					for (int line = 0; line < height; ++line) {
						...
						for (int i = 0; i < width; ++i) {
							if (littleEndian) {
								r = DataUtil.readUnsignedShortLittleEndian(lineOutput, i*6);
								g = DataUtil.readUnsignedShortLittleEndian(lineOutput, i*6+2);
								b = DataUtil.readUnsignedShortLittleEndian(lineOutput, i*6+4);
							} else {
								r = DataUtil.readUnsignedShortBigEndian(lineOutput, i*6);
								g = DataUtil.readUnsignedShortBigEndian(lineOutput, i*6+2);
								b = DataUtil.readUnsignedShortBigEndian(lineOutput, i*6+4);
							}
							if (r == tR && g == tG && b == tB)
	...
	private static class DirectIntegerReader extends ScanLineReader {
		public DirectIntegerReader(AsyncWork<? extends ColorModel, Exception> colorModel, int width, int height, int bytesPerLine, boolean littleEndian, boolean bottomUp) {
			super(colorModel);
			this.width = width;
			this.height = height;
			this.bottomUp = bottomUp;
			...
			new Task.Cpu<Void, NoException>("Scan Line Reader", io.getPriority()) {
				...
				public Void run() {
					for (int line = 0; line < height; line++) {
						...
						if (littleEndian)
							for (int i = 0; i < width; ++i)
								buffer[off+i] = DataUtil.readIntegerLittleEndian(lineBuffer, i*4);
						else
							for (int i = 0; i < width; ++i)
								buffer[off+i] = DataUtil.readIntegerBigEndian(lineBuffer, i*4);
					}
					done.unblock();
					return null;
				}
